...
static uint8_t curr_image_idx = 0;
static int16_t curr_image_angle = 0;
static lv_obj_t *images[12];
static unsigned long ani_next_frame_ms;
...
void setup()
{
  ...
  else
  {
    ...
    images[8] = ui_Image9;
    images[9] = ui_Image10;
    images[10] = ui_Image11;
    images[11] = ui_Image12;

    Serial.println("Setup done");
    ani_next_frame_ms = millis() + (1000 / ANIMATION_FPS);
  }
}
...
void loop()
{
  ...
  if (ms < 200)
  {
    ani_next_frame_ms = ms;
    calculate_next_get_timeinfo();
  }

  // handle animation
  if (ms >= ani_next_frame_ms)
  {
    // Serial.printf("ms: %d, ani_next_frame_ms: %d\n", ms, ani_next_frame_ms);
    uint8_t next_image_idx = curr_image_idx + 1;
    if (next_image_idx > 11)
    {
      next_image_idx = 0;
    }

    // show next image then hide current image
    lv_obj_clear_flag(images[next_image_idx], LV_OBJ_FLAG_HIDDEN);
    lv_obj_add_flag(images[curr_image_idx], LV_OBJ_FLAG_HIDDEN);

    curr_image_idx = next_image_idx;
    ani_next_frame_ms += (1000 / ANIMATION_FPS);
  }
